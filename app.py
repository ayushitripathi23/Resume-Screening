...
image = Image.open('Images/resume bg.jpg')
...
# Checking for Multiple Job Descriptions, and if more than one Job Descriptions are available, it asks user to select one as well.
if len(Jobs['Name']) <= 1:
...
if len(Jobs['Name']) > 1:
    ...
    if option_yn == 'YES':
        ...
        fig = go.Figure(data=[go.Table(header=dict(values=["Job No.", "Job Desc. Name"], line_color='White',
                                                   fill_color='#00416d',font=dict(color='white', size=16)),
...
fig2 = px.bar(Ranked_resumes, x=Ranked_resumes['Name'], y=Ranked_resumes['Scores'], color='Scores',
...
st.markdown("""  Wordcloud representation shows the Topic Number and the Top Keywords that contstitute a Topic. This is used to cluster the resumes.  """)
...
if option_2 == 'YES':
    ...
    fig = go.Figure(data=[go.Table(
        header=dict(values=["Resume"], fill_color='#ff1a1a', align='center', font=dict(color='white', size=16)),
        cells=dict(values=[str(value)], fill_color='#ffcccb', align='left'))])
