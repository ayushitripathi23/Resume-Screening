...
# load the spacy module and create a nlp object. This need the spacy en module to be present on the system.
nlp = spacy.load('en_core_web_sm')

# proces to remove stopwords form a file, takes an optional_word list for the words that are not present in the stop words but the user wants them deleted.
...
def remove_tags(text, postags=['PROPN', 'NOUN', 'ADJ', 'VERB', 'ADV']):
    #Takes in Tags which are allowed by the user and then elimnates the rest of the words based on their Part of Speech (POS) Tags.
    filtered = []
    str_text = nlp(" ".join(text))
